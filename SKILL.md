...
7. **设置权限**：将多维表格设置为所有人可编辑，不限制查看和写入权限

#### 写入记录

读取 `~/.openclaw/workspace/emby-bitable.json` 获取 `app_token` 和 `table_id`。

**重要：写入记录时以发送指令的用户身份操作，发起人字段记录该用户的飞书名称。**
...
**严格按以下格式输出，不要添加明细、不要列出具体账号：**

统计时从多维表格中读取记录，按**发起人**和**续费天数**分组，统计每个人每种天数续费了几个账号。
...
**重要规则：**
1. 每行格式固定为：`发起人 → 续费X天 → N个`
2. 按发起人分组，同一个人不同天数分别列一行
3. **不要**列出具体账号名
4. **不要**列出明细
5. **不要**列出总续费用户数
6. 注册统计格式为：`发起人 → 注册 → N个`
7. 其他操作只统计总次数，用 `|` 分隔写在一行
